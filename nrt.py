...
# Configure FRR and OSPF in namespace
def configure_ospf_in_namespace(ns_name, iface, ip, prefix, m1, m2, client, up, area, hi, di):
    ...
    print(f"Configuring FRR and OSPF in namespace {ns_name} for interface {iface}")
    
    # Create directory structure for FRR in the namespace
...
# the instances can be added. Instances are separated by space.
#
zebra=yes
bgpd=no
ospfd=yes
...
watchfrr_options="--netns=$FRR_NAMESPACE"
...
ip netns exec {ns_name} /usr/lib/frr/zebra -d -N {ns_name} -f /etc/netns/{ns_name}/frr/frr.conf
ip netns exec {ns_name} /usr/lib/frr/ospfd -d -N {ns_name} -f /etc/netns/{ns_name}/frr/frr.conf
"""
    ...
    # Verify FRR is running in the namespace
    ps_output = run_cmd(['ps', 'aux'], capture_output=True, text=True).stdout
    if f"zebra -d -N {ns_name}" in ps_output and f"ospfd -d -N {ns_name}" in ps_output:
        print(f"FRR daemons are running in namespace {ns_name}")
    else:
        print(f"Warning: FRR daemons may not be running properly in namespace {ns_name}")
...
# Check OSPF status in namespace
def show_ospf_status_in_namespace(ns_name):
    print(f'\n=== Checking OSPF status in namespace {ns_name} (30s timeout) ===')
    
    # Create a script to run vtysh in the namespace
    vtysh_script = f"""#!/bin/bash
export FRR_NAMESPACE={ns_name}
ip netns exec {ns_name} vtysh -c 'show ip ospf neighbor'
"""
    with open('/tmp/vtysh_ospf.sh', 'w') as f:
        f.write(vtysh_script)
    
    run_cmd(['chmod', '+x', '/tmp/vtysh_ospf.sh'], check=True)
    
    # Check OSPF neighbor status
    success = False
    for _ in range(30):
        result = run_cmd(['/tmp/vtysh_ospf.sh'], capture_output=True, text=True)
        if 'Full/DR' in result.stdout:
            success = True
    ...
    # Create a script to show the routing table
    vtysh_route_script = f"""#!/bin/bash
export FRR_NAMESPACE={ns_name}
ip netns exec {ns_name} vtysh -c 'show ip route'
"""
    with open('/tmp/vtysh_route.sh', 'w') as f:
        f.write(vtysh_route_script)
    
    run_cmd(['chmod', '+x', '/tmp/vtysh_route.sh'], check=True)
    
    # Run the script
    route_result = run_cmd(['/tmp/vtysh_route.sh'], capture_output=True, text=True)
    print(route_result.stdout)
    
    # Also show the kernel routing table
    print(f'\n=== Kernel Routing Table in namespace {ns_name} ===')
    route_output = run_in_namespace(ns_name, ['ip', 'route'], capture_output=True, text=True).stdout
    print(route_output)
    
    # Clean up
    run_cmd(['rm', '/tmp/vtysh_ospf.sh', '/tmp/vtysh_route.sh'], check=False)
