...
def configure_ospf_in_namespace(ns_name, iface, ip, prefix, m1, m2, client, up, area, hi, di):
    ...
    # Create FRR config directories in the namespace if they don't exist
...
"""
    ...
    # Create empty vtysh.conf file to avoid the error
    with open(f'/etc/netns/{ns_name}/frr/vtysh.conf', 'w') as f:
        f.write("# Automatically generated by nrt.py\n")
    ...
    # Instead of trying to restart FRR in the namespace (which might not work),
    # we'll use a simpler approach by directly configuring OSPF using vtysh commands
    # in the default namespace
    
    # Use vtysh to configure OSPF
    run_cmd(['vtysh', '-c', 'configure terminal', 
             '-c', 'router ospf', 
...
def show_ospf_status_in_namespace(ns_name):
    ...
    # Use vtysh in the default namespace instead of in the namespace
    # This avoids issues with vtysh.conf in the namespace
    for _ in range(30):
    ...
    # Use ip route in the namespace to show routes
    # This is more reliable than using vtysh in the namespace
    route_output = run_in_namespace(ns_name, ['ip', 'route'], capture_output=True, text=True).stdout
    print(route_output)
    
    # Also try to show the FRR routing table from the default namespace
    print("\n=== FRR Routing Table ===")
